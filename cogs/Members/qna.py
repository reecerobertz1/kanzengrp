...
class QnAView(discord.ui.View):
    ...
    async def view_faqs(self, interaction: discord.Interaction, button: discord.ui.Button):
        faq_embed = discord.Embed(title="Frequently Asked Questions", description="Here are some common questions and answers:", color=discord.Color.blue())
        faq_embed.add_field(name="Q: How do I apply?", value="A: You can apply by clicking the 'Apply' button in the recruit channel.", inline=False)
        faq_embed.add_field(name="Q: What are the requirements?", value="A: Please refer to the recruit announcement for specific requirements.", inline=False)
        faq_embed.add_field(name="Q: Can I re-apply if I get declined?", value="A: Yes, you have 2 attempts to apply for each recruit.", inline=False)
...
class QnAModal(discord.ui.Modal, title="Ask a Question"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        await question_channel.send(embed=embed, view=AnswerView(self.bot, asker=interaction.user, question=self.question.value))
        await interaction.response.send_message("Your question has been submitted! A staff member will get back to you as soon as possible.", ephemeral=True)
...
class FollowUpModal(discord.ui.Modal, title="Ask a Follow-Up Question"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        await interaction.response.send_message("Your follow-up question has been submitted! A staff member will get back to you as soon as possible.", ephemeral=True)
