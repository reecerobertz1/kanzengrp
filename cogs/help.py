    ...
    async def help(self, ctx):
        ...
        async def dropdown_callback(interaction: discord.Interaction):
            ...
            if selected_category == categories[0]:
                ...
                embed.add_field(name="+warn", value="<:reply:1125269313432059904> Give a warning to a member in the `[secret]` server", inline=False)
                embed.add_field(name="+steal", value="<:reply:1125269313432059904> Steal an emoji from any server", inline=False)
                embed.set_thumbnail(url=ctx.guild.icon)
